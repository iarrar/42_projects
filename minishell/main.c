...
/*   Updated: 2024/02/06 00:03:25 by iarrar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int get_status(t_command *cmd)
{
    ...
    if (i > 0)
        ...
        return -1; // or any default value you choose for an empty command array
}

void free_free(char **cmds, char *cmd, t_command *pipe_cmd)
{
    if (cmds != NULL)
        ft_freetabs(cmds); // Free array of strings

    if (cmd != NULL)
        free(cmd); // Free the main command string

    if (pipe_cmd != NULL)
        ft_free_command(pipe_cmd); // Free command structures in the array
}
